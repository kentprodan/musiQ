...
- [x] Library page: sorting/filtering (title bar search box + sort field/direction controls), album art (local files only — embedded picture extracted via `lofty`, cached to disk, shown per-track; Plex/Navidrome album art was already done, see Core section) — **built and unit-tested but not yet visually verified live**, see note below
- [ ] **User (or a future session) to visually verify title bar search, Library sort, and local album art in the running app** — blocked this session by `TextInputHost.exe` being reported as the foreground window for every computer-use click/keystroke attempt, for the whole screen, not just this app; access to that process was (correctly) denied since it's a normal system component, not something to grant blanket control over. Code builds clean, `cargo test -p musiq-core` is 18/18, but none of this was seen actually running.
