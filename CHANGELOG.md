...
### Added
- **Title bar search box**: added an `AutoSuggestBox` to the `TitleBar.Content` region in `MainWindow.xaml`. Confirmed via the `TitleBar` control's dev spec (`microsoft/microsoft-ui-xaml`) that the newer declarative `TitleBar` control (Windows App SDK 1.7+, which this app already uses for its custom title bar) auto-manages `InputNonClientPointerSource` passthrough regions for anything placed in `Content`/`LeftHeader`/`RightHeader` — unlike the older manual `Grid`-based title bar pattern, where the app itself has to calculate and set passthrough rects by hand. This means the item that had been sitting on `TODO.md` since the manual-title-bar days ("interactive content needs `InputNonClientPointerSource` passthrough regions") turned out to already be solved by the control this app was built on — no extra plumbing needed, just placing the control in `Content`. The box only shows on the Library page (hidden/cleared via a `Frame.Navigated` handler on every other page) and filters live via `LibraryViewModel.FilterText`.
- **Library page sort**: `LibraryViewModel` now filters (`FilterText`, matched against title/artist/album) and sorts (`SortField`: Title/Artist/Album/Duration, plus `SortDescending`) the displayed track list on top of the full scanned library, via CommunityToolkit.Mvvm's `On<Prop>Changed` partial hooks. A `ComboBox` + direction-toggle `Button` on `LibraryPage` drive it.
- **Local album art**: new `core/musiq-core::art` module extracts embedded cover art via `lofty`'s `Tag::pictures()`, caching the bytes to a file in an `art-cache` folder next to the SQLite database (keyed by track ID, extension inferred from the picture's `MimeType`) so repeat lookups are a cache-hit file-exists check instead of re-probing the audio file. Exposed as `Library::track_art_path`/`Library.TrackArtPath` through UniFFI. `LibraryViewModel.RefreshAsync` fetches every displayed track's art in parallel (`Task.WhenAll`) and attaches it to `TrackItem.ArtUrl` as a `file:///`-style absolute URI (via `new Uri(path).AbsoluteUri`, not a raw Windows path) so XAML's `Image.Source` converter can bind to it directly, matching the Plex/Navidrome album-art pattern from earlier. `LibraryPage`'s `ListView` gained a 40x40 art column.
- Verified: `cargo test -p musiq-core` (18/18 passing, no new test for `art.rs` itself — this crate's tag-*reading* logic against real audio files has never had synthetic-fixture unit tests, same as `tags.rs`'s tag-*writing* path; both are exercised against real files instead) and `dotnet build` both clean (only the pre-existing `MVVMTK0045` advisory warnings, tracked separately). Generated 3 test MP3s via `ffmpeg` (two with embedded cover art of different colors, one without) for manual verification.

### Known issue
- **Could not complete interactive live verification of this session's changes.** The desktop environment's `TextInputHost.exe` (Windows' input-experience host — a normal, always-running system process, not something triggered by this app) was reported as the foreground window for every click/keystroke attempt via computer-use, for the entire screen, not just the app window — blocking all interaction. Requesting access to it was denied (it's a system process, correctly not something to blanket-grant). Code builds clean and the Rust test suite passes, but the title-bar search box, sort controls, and local album-art rendering have **not** been visually confirmed working in the running app. Needs a manual check next time the desktop is interactive.
